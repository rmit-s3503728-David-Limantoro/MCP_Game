...
public class Game extends JFrame {
	...
	public Game() throws Exception {
		...
		gih = new GameInputHandler(player, panel, this);
		panel.add(up);
		panel.add(down);
		panel.add(left);
		panel.add(right);
		...
		settingButton.addActionListener(gih);

		panel.addKeyListener(new GameInputHandler(player, panel, this));
	...
	public void setUpGameOnLoad(Login log, Score s, Cell[][] cel2d, Cell[] c, ArrayList<Monster> m, Player p,
			Setting set) {
		login = log;
		score = s;
		grid.cells2D = cel2d;
